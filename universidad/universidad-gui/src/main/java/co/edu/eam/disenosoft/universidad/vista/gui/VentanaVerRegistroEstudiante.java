...
import co.edu.eam.disenosoft.universidad.contrloadores.ControladorVentanaVerRegistroEstudiante;
...
public class VentanaVerRegistroEstudiante extends javax.swing.JInternalFrame {
	...
	private void btnQuitarAsignaturaActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnQuitarAsignaturaActionPerformed
		try {
			...
			String est = tfcedulaestudiante.getText();
			Estudiante estud = controlador.buscarEstudiante(est);
			int pos = tablaHorario.getSelectedRow();
			DefaultTableModel dtm = (DefaultTableModel) tablaHorario.getModel();
			if (pos >= 0) {
				String asignat = (String) dtm.getValueAt(pos, 0);
				controlador.eliminarRegistroCursoEstudiante(estud, asignat);
				JOptionPane.showMessageDialog(null, "Se ha eliminado del curso");
				llenarTablaRegistroEst(estud);
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	/**
	 * lista la tabla con los cursos registrados por un estudiante
	 * 
	 * @param e
	 *            el estudiante
	 * @throws Exception
	 ...
	 */
	public void llenarTablaRegistroEst(Estudiante e){
		try {
			DefaultTableModel dtm = (DefaultTableModel) tablaHorario.getModel();
			dtm.setRowCount(0);
			String cadena = "";
			List<RegistroCurso> lista = controlador.listarRegistrosEstudiante(e);
